...
ALGO_CSV_CANDIDATES = [(['BP-16', 'BP-Scalar'], 'BP (Scalar)'), (['BP-SIMD-16', 'BP-SIMDComp'], 'BP (SIMD)'), (['BP-FastLanes-16', 'BP-FastLanes'], 'BP (FastLanes)'), (['BP-SIMT-16'], 'BP (SIMT)'), (['BP+Prune', 'BP+Prune-Scalar'], 'BP-Prune-RMQ (Scalar)'), (['BP+Prune-SIMD', 'BP+Prune-SIMDComp'], 'BP-Prune-RMQ (SIMD)'), (['BP+Prune-FastLanes'], 'BP-Prune-RMQ (FastLanes)'), (['BP+Prune-SIMT'], 'BP-Prune-RMQ (SIMT)')]
...
FONTSIZE = 22
EXAMPLE_DATA = [('Example', {'BP (Scalar)': {'ratio': 10.0, 'encode_ns': 190.0, 'decode_ns': 70.0}, 'BP (SIMD)': {'ratio': 10.0, 'encode_ns': 110.0, 'decode_ns': 36.0}, 'BP (FastLanes)': {'ratio': 10.0, 'encode_ns': 120.0, 'decode_ns': 38.0}, 'BP (SIMT)': {'ratio': 10.0, 'encode_ns': 90.0, 'decode_ns': 30.0}, 'BP-Prune-RMQ (Scalar)': {'ratio': 8.0, 'encode_ns': 160.0, 'decode_ns': 44.0}, 'BP-Prune-RMQ (SIMD)': {'ratio': 8.0, 'encode_ns': 100.0, 'decode_ns': 40.0}, 'BP-Prune-RMQ (FastLanes)': {'ratio': 8.0, 'encode_ns': 200.0, 'decode_ns': 48.0}, 'BP-Prune-RMQ (SIMT)': {'ratio': 8.0, 'encode_ns': 84.0, 'decode_ns': 28.0}})]
...
def plot_simd():
    ...
    if merged:
        ...
        if n_found < n_algos:
            print(f'  Warning: expected {n_algos} algorithms; found at most {n_found}.', file=sys.stderr)
    datasets = [m[0] for m in merged]
    n = len(datasets)
    x = np.arange(n, dtype=float)
    bar_w = _BAR_W
    fig_w = 1.2 * min(22, 10 + max(n, 1) * 0.45)
    plt.rcParams.update({'font.size': FONTSIZE, 'axes.titlesize': FONTSIZE, 'axes.labelsize': FONTSIZE, 'xtick.labelsize': FONTSIZE, 'ytick.labelsize': FONTSIZE, 'legend.fontsize': FONTSIZE})
    fontsize = FONTSIZE
    ...
    def _draw_grouped_bars(ax, key):
        for j in range(n):
            ...
            for i in range(N_IMPL):
                ...
                _bar_with_err(ax, x_left, vp, bar_w, yerr_p, color=c, edgecolor='none', linewidth=0)
                _bar_with_err(ax, x_right, vr, bar_w, yerr_r, color=c, edgecolor='white', linewidth=1.0, hatch='///')
    ax1 = axes[0]
    ...
    for i in range(N_IMPL):
        c = GROUP_COLORS[i]
        legend_handles.append(Patch(facecolor=c, edgecolor='white', linewidth=1.0, hatch='///', label=ALGOS[i + N_IMPL]))
    fig.legend(legend_handles, [h.get_label() for h in legend_handles], loc='upper center', bbox_to_anchor=(0.5, 1.02), ncol=4, fontsize=fontsize, frameon=True, fancybox=False, edgecolor='0.8', labelspacing=0.1, handletextpad=0.1, columnspacing=0.1)
    ...
    def _draw_avg_grouped(ax, vals, stds):
        for i in range(N_IMPL):
            ...
            _bar_with_err(ax, x_avg[i] - width_pair / 2, vp, width_pair, ye_p if ye_p > 0 else None, color=c, edgecolor='none', linewidth=0)
            _bar_with_err(ax, x_avg[i] + width_pair / 2, vr, width_pair, ye_r if ye_r > 0 else None, color=c, edgecolor='white', linewidth=1.0, hatch='///')
    ax1 = axes2[0]
    ...
    ax3.set_title('(c) Decompression Time', fontsize=fontsize)
    ax3.tick_params(axis='y', labelsize=fontsize)
    fig2.legend(legend_handles, [h.get_label() for h in legend_handles], loc='upper center', bbox_to_anchor=(0.5, 1.02), ncol=4, fontsize=fontsize, frameon=True, fancybox=False, edgecolor='0.8', labelspacing=0.1, handletextpad=0.1, columnspacing=0.1)
    plt.tight_layout(rect=[0, 0, 1, 0.85])
