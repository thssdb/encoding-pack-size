...
DATASET_FILES = ['City-temp.csv', 'Wind-Speed.csv', 'IR-bio-temp.csv', 'PM10-dust.csv', 'Air-pressure.csv', 'Dew-point-temp.csv', 'Stocks-UK.csv', 'Stocks-USA.csv', 'Stocks-DE.csv', 'Bitcoin-price.csv', 'Bird-migration.csv', 'Food-price.csv', 'Blockchain-tr.csv', 'TY-Fuel.csv', 'TY-Transport.csv']
...
def plot_pack_size_limited_vs_full():
    ...
    ax.set_xlabel('Threshold $\\lambda$', fontsize=fontsize)
    ...
    ax.tick_params(axis='both', labelsize=fontsize)
    handles = [Line2D([0], [0], color=LINE_KW['color'], linestyle='-', linewidth=2, marker='o', markersize=8, label=limited_label), Line2D([0], [0], color=HLINE_KW['color'], linestyle='--', linewidth=2, label=full_label)]
    fig.legend(handles=handles, loc='upper center', bbox_to_anchor=(0.5, 1.02), ncol=len(handles), fontsize=fontsize)
