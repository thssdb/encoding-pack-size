#!/usr/bin/env python3
"""
2×2 figure combining:
  (a) Scatter: BitWidthDiffAbsMean vs improvement (raw points; same x/y labels as before)
  (b) Scatter: MaxMinBitWidthDiff vs improvement (same style as (a))
  (c) RF predicted vs actual (test): features BitWidthStd + MaxMinBitWidthDiff only
  (d) RF predicted vs actual (test): 10 features (same as plot_predicted_vs_actual.py)

Title, axis labels, tick labels, and inset annotation boxes all use fontsize 22.

Run: python3.11 scripts/plot_four_panel_improvement_figure.py
Output: figure_for_paper/fig_four_panel_improvement.png and .eps
"""
...
# Same 10-feature set as plot_predicted_vs_actual.py
FEATURE_COLS_10 = [
    "MaxMinBitWidthDiff",
...
def plot_scatter_on_ax(ax, df, col, xlabel, title_fs, label_fs, tick_fs, panel_title, annot_fs):
    """Raw scatter: each chunk one point; no grid."""
    ...
    if len(raw) >= 10:
        ...
        r2 = r * r
        ax.text(
            0.02, 0.98, f"ρ = {r:.3f}  R² = {r2:.3f}",
            transform=ax.transAxes, fontsize=annot_fs, verticalalignment="top",
...
def main():
    ...
    # (a) BitWidthDiffAbsMean — scatter (raw x/y); axis labels unchanged
    plot_scatter_on_ax(
        axes[0, 0], df, "BitWidthDiffAbsMean",
        "mean of bit width delta",
        title_fs=title_fs, label_fs=label_fs, tick_fs=tick_fs,
        panel_title="(a) The impact of smoothness",
        annot_fs=annot_fs,
    )

    # (b) MaxMinBitWidthDiff — scatter (same style as (a))
    plot_scatter_on_ax(
        axes[0, 1], df, "MaxMinBitWidthDiff",
        "bit length range",
        title_fs=title_fs, label_fs=label_fs, tick_fs=tick_fs,
        panel_title="(b) The impact of bit length range",
        annot_fs=annot_fs,
    )

    # Square axes box for top-row scatter panels (data aspect not forced to 1:1)
    axes[0, 0].set_box_aspect(1)
    axes[0, 1].set_box_aspect(1)

    # Shared split for (c) and (d) — same row indices for both models
    ...
    # (c) 2 features: BitWidthStd + MaxMinBitWidthDiff
    sc2 = StandardScaler()
    ...
    plot_pred_vs_actual_on_ax(
        axes[1, 0], y_test2, pred2,
        panel_title="(c) The prediction of 2 features",
        label_fs=label_fs, tick_fs=tick_fs, title_fs=title_fs, annot_fs=annot_fs,
    )
    ...
    plot_pred_vs_actual_on_ax(
        axes[1, 1], y_test10, pred10,
        panel_title="(d) The prediction of 10 features",
        label_fs=label_fs, tick_fs=tick_fs, title_fs=title_fs, annot_fs=annot_fs,
    )
