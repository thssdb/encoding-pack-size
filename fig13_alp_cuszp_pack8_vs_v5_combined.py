#!/usr/bin/env python3
"""
Combine ALP and CuSZp2 pack8 vs Prune-RMQ in one figure: 1×3 subplots; per dataset,
four bars in two tight bar groups (ALP / ALP（Prune-RMQ）) and (CuSZp2 / CuSZp2 (Prune-RMQ)).

Bar style matches fig_vary_pack_size_sort_final.py: shared color within each method family,
Prune-RMQ hatched (///), black edges.

Error bars: per-dataset bars use nearest-k (k=5) min–max spread among *other* datasets for that
method (same as fig_vary_pack_size.py); average figure uses std dev across datasets per method.

Also writes a 1×3 average figure with the same grouped layout.

Depends on the same CSV outputs as fig_alp_pack8_vs_v5.py and fig_cuszp_pack8_vs_v5.py.
"""
import os
import sys
...
ALP_CSV = [
    (["ALP"], "ALP"),
    (["ALP+V5pack", "ALP+optimal-V5", "ALP optimal V5"], "ALP（Prune-RMQ）"),
]
ALP_ALGOS = [x[1] for x in ALP_CSV]

CUSZP_CSV = [
    (["cuSZp-cpu-plain-simplified", "cuSZp-cpu", "CuSZp-cpu"], "CuSZp2"),
    (["cuSZp-cpu+V5pack", "cuSZp+V5pack", "CuSZp+V5pack"], "CuSZp2 (Prune-RMQ)"),
]
CUSZP_ALGOS = [x[1] for x in CUSZP_CSV]

# Same style as fig_vary_pack_size_sort_final.py: one color per method family;
# baseline solid, Prune-RMQ hatched (///). CuSZp2 uses a muted blue–gray (not bright red).
GROUP_COLOR_ALP = "#9467bd"
GROUP_COLOR_CUSZP = "#5d8499"
ALGO_FACE_COLOR = {
    "ALP": GROUP_COLOR_ALP,
    "ALP（Prune-RMQ）": GROUP_COLOR_ALP,
    "CuSZp2": GROUP_COLOR_CUSZP,
    "CuSZp2 (Prune-RMQ)": GROUP_COLOR_CUSZP,
}
# True -> hatch for Prune-RMQ variant (matches Sort bars in sort_final)
ALGO_USE_HATCH = {
    "ALP": False,
    "ALP（Prune-RMQ）": True,
    "CuSZp2": False,
    "CuSZp2 (Prune-RMQ)": True,
}

# Bar order: two tight pairs (ALP + ALP-Prune) | (CuSZp2 + CuSZp2-Prune) => 4 bars / cluster
ALGO_ORDER = ("ALP", "ALP（Prune-RMQ）", "CuSZp2", "CuSZp2 (Prune-RMQ)")
BAR_HATCH_PATTERN = "///"

# Error bars: per-dataset bars — pool = same algo on *other* datasets, then min/max among k values closest to this bar (fig_vary_pack_size.py).
...
def _bar_kwargs(algo, fc):
    """No black frame; Prune-RMQ uses white hatch (mpl draws hatch with edgecolor)."""
    kw = {"color": fc}
...
def nearest_k_minmax_errors(vals, m, k, log_scale=False):
    """Distance-to-m among up to k finite values → min/max; return lower/upper error from m (fig_vary_pack_size)."""
    if k is None or k < 1:
        return 0.0, 0.0
...
def _grouped_bar_xcenters(xc, w, gap_in, gap_between_groups):
    """Four bar centers at cluster center xc: pair1 tight, gap, pair2 tight."""
...
def _plot_grouped_metrics(
    ...
    *,
    ...
    """One row of axes: each panel plots all datasets with 4 grouped bars."""
    n = len(merged_alp_i)
    if n == 0:
        return 1.0, 0.14
    ...
    titles = (
        f"({letters[0]}) Compression Ratio",
        f"({letters[1]}) Compression Time",
        f"({letters[2]}) Decompression Time",
    )
        ...
        for k in range(n):
            ...
            for xi, algo, v in zip((c0, c1, c2, c3), ALGO_ORDER, vals):
                ...
                if (
                    np.isfinite(v)
                    and len(pool) > 0
                    and ERRORBAR_NEAREST_K is not None
                    and int(ERRORBAR_NEAREST_K) >= 1
                ):
    ...
    # (a) Compression ratio: default floor 3; extend downward if error bars need it
    ratio_bottom = 3.0
    if ratio_y_low:
...
def plot_combined():
    ...
    if not merged_alp_i:
        print(
            "No datasets with both ALP and CuSZp paired outputs; run both test harnesses.",
            file=sys.stderr,
        )
        return
    ...
    # Legend below top margin; x=0.5 keeps horizontal center (figure coordinates)
    plt.tight_layout(rect=(0.06, 0.10, 1.0, 0.74))
    fig.legend(
    ...
    # Averages: same 1×3 layout, single grouped cluster per panel
    bar_w = 0.16
    gap_in = 0.014
    ...
    for ax, title, key, yl in zip(axes2, titles, keys, YLABELS_METRICS):
        for xi, algo in zip((c0, c1, c2, c3), ALGO_ORDER):
            if algo in ALP_ALGOS:
                vals = [
                    float(d[algo][key])
                    for _abbr, d in merged_alp_i
                    if algo in d and np.isfinite(d[algo][key])
                ]
            else:
                vals = [
                    float(d[algo][key])
                    for _abbr, d in merged_cuszp_i
                    if algo in d and np.isfinite(d[algo][key])
                ]
