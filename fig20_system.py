"""
System comparison figure from TsFile pack-size test output:
  AllNo8PacksizeOptimal.TsFilePackSize8VsOptimalComparisonTest
  -> output_tsfile_packsize_comparison/tsfile_comparison.csv

Three subplots: compression ratio; write split (I/O bottom with method-color fill + hatch, CPU top solid);
read split (same), all in ns/point (mean over allowed datasets).

Java CSV may include Write Encode (ns), Write IO (ns), Read IO (ns), Read Decode (ns); if absent,
(b)(c) fall back to a single segment per bar.

When the Java test uses per-page disk I/O (segmented write/read), Write IO / Read IO columns are
sums of many small operations; encode/decode columns stay CPU-oriented.
"""
import os
import numpy as np
...
# Only these datasets (must match `Dataset` column and tsfile name stems from the Java test).
dataset_mapping = {
    'City-temp.csv': 'CT',
    'Wind-Speed.csv': 'WS',
    'IR-bio-temp.csv': 'IR',
...
# CSV Mode -> legend label (aligned with paper)
MODE_TO_ALGO = {
    'PackSize8': 'Sprintz',
    'OptimalPackSize': 'Sprintz-Prune-RMQ',
}
ALGOS = ['Sprintz', 'Sprintz-Prune-RMQ']
COLORS = ['#2ca02c', '#17becf']
# Bottom stack = I/O: same fill color as method + black hatch; top = CPU solid same color.
IO_HATCH = '///'
LEGEND_CPU_IO_FILL = 'white'

# Placeholder if CSV missing or incomplete
EXAMPLE_BY_ALGO = {
    'Sprintz': {
        'compression_ratio': 6.5,
        'compression_ratio_std': 0.4,
        'write_enc_pp': 95.0,
        'write_enc_std': 12.0,
        'write_io_pp': 25.0,
        'write_io_std': 4.0,
        ...
        'read_io_pp': 25.0,
        'read_io_std': 4.0,
        'write_total_std': 14.0,
        'read_total_std': 9.0,
    },
    ALGOS[1]: {
        'compression_ratio': 7.0,
        'compression_ratio_std': 0.35,
        'write_enc_pp': 170.0,
        'write_enc_std': 18.0,
        'write_io_pp': 30.0,
        'write_io_std': 5.0,
...
def load_tsfile_comparison():
    """
    (a) Mean (Points*8)/TsFile Size per algorithm.
    (b)(c) ns/point splits when Java exports Encode/IO/Decode columns; else totals only on bottom.
    """
    ...
    for algo in ALGOS:
        ...
        out[algo] = {
            'compression_ratio': comp_mean,
            'compression_ratio_std': comp_std,
            'write_enc_pp': write_enc_pp,
            'write_enc_std': write_enc_std,
            'write_io_pp': write_io_pp,
            'write_io_std': write_io_std,
            'read_dec_pp': read_dec_pp,
            'read_dec_std': read_dec_std,
            'read_io_pp': read_io_pp,
            'read_io_std': read_io_std,
            'write_total_std': write_total_std,
            'read_total_std': read_total_std,
            'has_split': has_split,
        }
    return out
...
def plot_system():
    ...
    # (b) Write stacked: bottom = disk write I/O (hatched), top = CPU encode (solid)
    ax2 = axes[1]
    ...
    tot_w_y = [
        (w_io[i] + w_enc[i]) if (has_split and w_io[i] > 0) else w_enc[i]
        for i in range(len(ALGOS))
    ]
    ax2.errorbar(
        x,
    ...
    # (c) Read stacked: bottom = I/O (hatched), top = CPU decode (solid)
    ax3 = axes[2]
    for i in range(len(ALGOS)):
    ...
    tot_r_y = [
        (r_io[i] + r_dec[i]) if (has_split and r_io[i] > 0) else r_dec[i]
        for i in range(len(ALGOS))
    ]
    ax3.errorbar(
        x,
    ...
    # ylim from stacked total + total std (single error bar per bar).
    wr_hi = [tot_w_y[i] + e_w_tot[i] for i in range(len(ALGOS))]
    rd_hi = [tot_r_y[i] + e_r_tot[i] for i in range(len(ALGOS))]
    _pad = 0.08 * max(max(wr_hi), max(rd_hi), 1e-9)
    bc_ymax = max(max(wr_hi), max(rd_hi), 1e-9) + _pad
    ...
    # Legend: white swatches — CPU (no hatch) vs I/O (diagonal hatch); bar colors follow x-axis method.
    legend_handles = [
        Patch(
            facecolor=LEGEND_CPU_IO_FILL,
            edgecolor='k',
            linewidth=0.5,
            hatch='',
            label='CPU Time',
