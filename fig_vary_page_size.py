...
# 定义8种不同的标记
markers = [ 'v', '^', trapezoid, parallelogram, heart, 's', 'v', heart_parametric]

# 算法顺序
algorithm_order = ['BP', 'BP-Prune', 'BP-Prune-RMQ', 'BP-All',  'Sprintz','Sprintz-Prune', 'Sprintz-Prune-RMQ',  'Sprintz-All']
...
for i, algorithm in enumerate(bp_group):
    ...
    ax1.plot(data['m'], data['Compression Ratio'],
...
ax1.set_title('(a) BP: Compression Ratio', fontsize=fontsize)
...
for i, algorithm in enumerate(bp_group):
    ...
    linestyle = '-' if 'All' in algorithm or 'RMQ' not in algorithm else '--'
    ax2.plot(data['m'], data['Encoding Time (MB/s)'],
             color=algorithm_palette[algorithm], linestyle=linestyle, marker=markers[i],
             markersize=7, linewidth=2.2, label=algorithm)
ax2.set_xscale('log', base=2)
# ax2.set_yscale('log')
ax2.set_ylim(9,11001)
ax2.set_ylabel('Time (ns/point)', fontsize=fontsize)
ax2.set_xlabel(r'Page Size $n$', fontsize=fontsize)
ax2.set_title('(c) BP: Compression Time', fontsize=fontsize)
...
for i, algorithm in enumerate(bp_group):
    ...
    linestyle = '-' if 'All' in algorithm or 'RMQ' not in algorithm else '--'
    ax3.plot(data['m'], data['Decoding Time (MB/s)'],
             color=algorithm_palette[algorithm], linestyle=linestyle, marker=markers[i],
             markersize=7, linewidth=2.2, label=algorithm)
ax3.set_xscale('log', base=2)
# ax3.set_yscale('log', base=2)
ax3.set_ylim(9,11001)
# ax3.set_ylim(450, 630)
ax3.set_ylabel('Time (ns/point)', fontsize=fontsize)
ax3.set_xlabel(r'Page Size $n$', fontsize=fontsize)
ax3.set_title('(e) BP: Decompression Time', fontsize=fontsize)
...
for i, algorithm in enumerate(sprintz_group):
    data = df_compression_ratio_reset[df_compression_ratio_reset['Algorithm'] == algorithm]
    linestyle = '-' if 'All' in algorithm or 'RMQ' not in algorithm else '--'
    ax4.plot(data['m'], data['Compression Ratio'],
...
ax4.set_xlabel(r'Page Size $n$', fontsize=fontsize)
ax4.set_title('(b) Sprintz: Compression Ratio', fontsize=fontsize)
...
for i, algorithm in enumerate(sprintz_group):
    data = df_encode_time_reset[df_encode_time_reset['Algorithm'] == algorithm]
    linestyle = '-' if 'All' in algorithm or 'RMQ' not in algorithm else '--'
    ax5.plot(data['m'], data['Encoding Time (MB/s)'],
             color=algorithm_palette[algorithm], linestyle=linestyle, marker=markers[i+3],
             markersize=7, linewidth=2.2, label=algorithm)
ax5.set_xscale('log', base=2)
# ax5.set_yscale('log', base=2)
ax5.set_ylim(9,11001)
ax5.set_ylabel('Time (ns/point)', fontsize=fontsize)
ax5.set_xlabel(r'Page Size $n$', fontsize=fontsize)
ax5.set_title('(d) Sprintz: Compression Time', fontsize=fontsize)
...
for i, algorithm in enumerate(sprintz_group):
    data = df_decode_time_reset[df_decode_time_reset['Algorithm'] == algorithm]
    linestyle = '-' if 'All' in algorithm or 'RMQ' not in algorithm else '--'
    ax6.plot(data['m'], data['Decoding Time (MB/s)'],
             color=algorithm_palette[algorithm], linestyle=linestyle, marker=markers[i+3],
             markersize=7, linewidth=2.2, label=algorithm)
ax6.set_xscale('log', base=2)
# ax6.set_yscale('log')
ax6.set_ylim(9,11001)
# ax6.set_ylim(450, 630)
ax6.set_ylabel('Time (ns/point)', fontsize=fontsize)
ax6.set_xlabel(r'Page Size $n$', fontsize=fontsize)
ax6.set_title('(f) Sprintz: Decompression Time', fontsize=fontsize)
...
fig.legend(by_label.values(), by_label.keys(), loc='upper center', ncol=4,
           labelspacing=0.1,
            handletextpad=0.1,
            columnspacing=0.1,
           fontsize=fontsize, bbox_to_anchor=(0.5, 0.98))
