...
def compute_prune_vs_vary_time_pct(bp_vary_dir='output_BP_vary_pack_size', bp_dir='output_BP', sprintz_vary_dir='output_Sprintz_vary_pack_size', sprintz_prune_dir='output_Sprintz_Prune_all_no8', pack_sizes=None):
    pack_sizes = pack_sizes or PACK_SIZES_VARY_UP_TO_1024
...
def plot_bp_sprintz_prune_vs_vary_pack_time(bp_vary_dir='output_BP_vary_pack_size', bp_dir='output_BP', sprintz_vary_dir='output_Sprintz_vary_pack_size', sprintz_prune_dir='output_Sprintz_Prune_all_no8', outpath='figure_for_paper/bp_sprintz_prune_vs_vary_pack_time.png', pack_sizes=None):
    common, vals_bp, vals_sp = compute_prune_vs_vary_time_pct(bp_vary_dir, bp_dir, sprintz_vary_dir, sprintz_prune_dir, pack_sizes)
    if common is None:
        print('No paired CSVs for BP/Sprintz prune vs vary-pack-size time plot.')
        return False
    common, vals_bp, vals_sp = map(list, zip(*sorted(zip(common, vals_bp, vals_sp), key=lambda t: t[0])))
...
def plot_combined_improvement_and_time(df, bp_vary_dir='output_BP_vary_pack_size', bp_dir='output_BP', sprintz_vary_dir='output_Sprintz_vary_pack_size', sprintz_prune_dir='output_Sprintz_Prune_all_no8', outpath='figure_for_paper/improvement_bp_sprintz_combined.png', pack_sizes=None):
    cols, bp_impr, sp_impr = compute_ratio_improvements(df)
    common_t, vals_bp_t, vals_sp_t = compute_prune_vs_vary_time_pct(bp_vary_dir, bp_dir, sprintz_vary_dir, sprintz_prune_dir, pack_sizes)
    ...
    rows = sorted(zip(abbrevs, bp_impr_v, sp_impr_v, bp_t_v, sp_t_v), key=lambda t: t[0])
...
def plot_percent_improvements(df, outpath='figure_for_paper/improvement_bp_sprintz.png', highlight_abbrevs=None):
    ...
    for b, v in zip(bars0, bp_impr):
        if np.isnan(v):
            continue
        ax0.text(b.get_x() + b.get_width() / 2, v + (top0 * 0.01 if top0 > 0 else 1.0), _fmt_bar_text_1dec(v), ha='center', va='bottom', rotation=90, fontsize=annot_fs)
    ax1 = axes[1]
    ...
    for b, v in zip(bars1, sp_impr):
        if np.isnan(v):
            continue
        ax1.text(b.get_x() + b.get_width() / 2, v + (top1 * 0.01 if top1 > 0 else 1.0), _fmt_bar_text_1dec(v), ha='center', va='bottom', rotation=90, fontsize=annot_fs)
    plt.tight_layout()
    os.makedirs(os.path.dirname(outpath) or '.', exist_ok=True)
    plt.savefig(outpath, dpi=300, bbox_inches='tight')
    try:
...
def main():
    ...
    parser.add_argument('--vary-pack-dir', default='output_BP_vary_pack_size', help='CSV dir from VaryPackSizeTest (sum times over pack sizes)')
    parser.add_argument('--bp-dir', default='output_BP', help='CSV dir for BP (Prune-RMQ) single-run times')
    ...
    parser.add_argument('--sprintz-vary-pack-dir', default='output_Sprintz_vary_pack_size', help='CSV dir from VaryPackSizeSprintzTest')
    parser.add_argument('--sprintz-prune-dir', default='output_Sprintz_Prune_all_no8', help='CSV dir for Sprintz+RMQ+Prune single-run times')
    parser.add_argument('--combined-plot', default='figure_for_paper/improvement_bp_sprintz_combined.png', help='2×2 figure: BP/Sprintz ratio improvement + prune-vs-vary encode time')
    ...
    plot_percent_improvements(df, outpath='figure_for_paper/improvement_bp_sprintz_add_dataset.png', highlight_abbrevs=IMPROVEMENT_HIGHLIGHT_ADD_DATASET_ABBREVS)
    plot_bp_sprintz_prune_vs_vary_pack_time(bp_vary_dir=args.vary_pack_dir, bp_dir=args.bp_dir, sprintz_vary_dir=args.sprintz_vary_pack_dir, sprintz_prune_dir=args.sprintz_prune_dir, outpath=args.vary_time_plot)
    plot_combined_improvement_and_time(df, bp_vary_dir=args.vary_pack_dir, bp_dir=args.bp_dir, sprintz_vary_dir=args.sprintz_vary_pack_dir, sprintz_prune_dir=args.sprintz_prune_dir, outpath=args.combined_plot)
